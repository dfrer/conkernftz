...
function summarizeTransformRule(rule) {
  ...
  const layer = rule.target?.layer;

  const layers = Array.isArray(rule.target?.layers) ? rule.target.layers : [];

  if (layer) targets.push(layer);

  layers.forEach((l) => { if (!targets.includes(l)) targets.push(l); });

  if (targets.length) parts.push(`Layers: ${targets.join(', ')}`);

  const values = Array.isArray(rule.target?.values) ? rule.target.values : [];
  ...
  if (cond) {
    ...
    if (Array.isArray(cond.anyOf) && cond.anyOf.length) condParts.push(`anyOf(${cond.anyOf.join(', ')})`);

    if (Array.isArray(cond.allOf) && cond.allOf.length) condParts.push(`allOf(${cond.allOf.join(', ')})`);

    if (Array.isArray(cond.noneOf) && cond.noneOf.length) condParts.push(`noneOf(${cond.noneOf.join(', ')})`);

    if (cond.not) condParts.push('not(...)');

    if (condParts.length) parts.push(`When ${condParts.join(' & ')}`);

  }

  if (rule.translate) {

    const tx = typeof rule.translate.x === 'number' ? rule.translate.x : 0;

    const ty = typeof rule.translate.y === 'number' ? rule.translate.y : 0;

    const mode = rule.translate.mode || 'add';

    parts.push(`Move (${tx}, ${ty}) [${mode}]`);

  }

  if (rule.rotate) {

    parts.push(`Rotate ${rule.rotate.degrees ?? 0}deg [${rule.rotate.mode || 'add'}]`);

  }

  if (rule.scale) {

    parts.push(`Scale ${rule.scale.factor ?? 1}x [${rule.scale.mode || 'multiply'}]`);

  }

  if (!parts.length) return 'No actions configured';

  return parts.join(' | ');

}

function transformTargetHasLayer(rule, layerName) {

  if (!rule || !rule.target || !layerName) return false;
...
function transformCountForLayer(layerName) {

  if (!layerName || !currentConfig || !currentConfig.rules || !Array.isArray(currentConfig.rules.transforms)) return 0;

  return currentConfig.rules.transforms.filter((rule) => transformTargetHasLayer(rule, layerName)).length;

}



function createTransformRuleId() {

  return `tf_${Date.now().toString(36)}_${Math.random().toString(16).slice(2, 8)}`;
...
let transformFilterLayer = null;

let transformListEl = null;

let transformBannerEl = null;

let transformSubtitleEl = null;



function getTransformLayerNames() {

  if (!currentConfig || !Array.isArray(currentConfig.layers)) return [];

  return currentConfig.layers
...
function prepareTransformRuleForEditing(rule) {

  const prepared = deepClone(rule || {});

  if (!prepared.id) prepared.id = createTransformRuleId();

  if (!prepared.target) prepared.target = {};

  const layerSet = new Set();

  if (typeof prepared.target.layer === 'string') layerSet.add(prepared.target.layer);

  if (Array.isArray(prepared.target.layers)) prepared.target.layers.forEach((l) => { if (typeof l === 'string') layerSet.add(l); });

  const layers = Array.from(layerSet).map((l) => l.trim()).filter((l) => l.length > 0);

  if (layers.length === 1) {

    prepared.target.layer = layers[0];

    prepared.target.layers = [layers[0]];

  } else if (layers.length > 1) {

    delete prepared.target.layer;

    prepared.target.layers = layers;

  } else {

    delete prepared.target.layer;

    delete prepared.target.layers;

  }

  if (!prepared.when) prepared.when = undefined;

  prepared.__notRaw = prepared.when && prepared.when.not ? JSON.stringify(prepared.when.not, null, 2) : '';
...
function ensureTransformModal() {

  if (transformModal) return transformModal;
  ...
  modal.innerHTML = `
    ...
    <div class="tf-modal-body" role="dialog" aria-modal="true" aria-label="Transformation rules">

      <div class="tf-modal-header">

        <div class="tf-modal-title">Transformation Rules</div>

        <button type="button" class="tf-close" aria-label="Close">×</button>

      </div>

      <div class="tf-modal-subtitle"></div>

      <div class="tf-banner" hidden></div>

      <div class="tf-modal-scroll">

        <div class="tf-rules"></div>

        <button type="button" class="tf-add-rule">+ Add rule</button>
      ...
      <div class="tf-modal-footer">
        ...
        <button type="button" class="tf-save">Apply Transforms</button>
  ...
  transformListEl = modal.querySelector('.tf-rules');

  transformBannerEl = modal.querySelector('.tf-banner');

  transformSubtitleEl = modal.querySelector('.tf-modal-subtitle');

  modal.addEventListener('input', handleTransformInput);

  modal.addEventListener('change', handleTransformChange);

  modal.addEventListener('click', handleTransformClick);
...
function openTransformModal(layerName) {

  if (!currentConfig) return;

  transformFilterLayer = layerName && layerName.trim() ? layerName.trim() : null;

  const source = (currentConfig.rules && Array.isArray(currentConfig.rules.transforms)) ? currentConfig.rules.transforms : [];

  transformDraft = source.map((rule) => prepareTransformRuleForEditing(rule));

  if (transformDraft.length === 0 && transformFilterLayer) {

    transformDraft.push(prepareTransformRuleForEditing({ target: { layer: transformFilterLayer } }));

  }

  const modal = ensureTransformModal();
...
function closeTransformModal() {
  ...
  transformModal.classList.remove('visible');

}



function renderTransformModal() {

  const modal = ensureTransformModal();

  if (!modal || !transformListEl) return;

  clearTransformBanner();

  clearTransformCardErrors();

  const layerNames = getTransformLayerNames();

  if (transformSubtitleEl) {

    transformSubtitleEl.textContent = transformFilterLayer

      ? `Filtering rules that include layer: ${transformFilterLayer}`

      : 'Move, rotate, or scale layers based on trait conditions.';

  }

  renderTransformCards(layerNames);

}



function renderTransformCards(layerNames) {

  if (!transformListEl) return;

  if (transformDraft.length === 0) {

    transformListEl.innerHTML = '<div class="tf-empty">No transform rules yet. Add one to start repositioning assets.</div>';

    return;

  }

  const cards = transformDraft.map((rule, idx) => buildTransformCardHtml(rule, idx, layerNames)).join('');

  transformListEl.innerHTML = cards;

}



function buildTransformCardHtml(rule, idx, layerNames) {

  const highlight = transformFilterLayer && transformTargetHasLayer(rule, transformFilterLayer) ? ' tf-card-highlight' : '';

  const description = escapeHtml(rule.description || '');

  const priorityStr = (typeof rule.priority === 'number' && Number.isFinite(rule.priority)) ? String(rule.priority) : '';

  const idStr = escapeHtml(rule.id || '');

  const valuesStr = Array.isArray(rule.target?.values) ? rule.target.values.join(', ') : '';

  const filenamesStr = Array.isArray(rule.target?.filenames) ? rule.target.filenames.join(', ') : '';

  const anyOfStr = Array.isArray(rule.when?.anyOf) ? rule.when.anyOf.join(', ') : '';

  const allOfStr = Array.isArray(rule.when?.allOf) ? rule.when.allOf.join(', ') : '';

  const noneOfStr = Array.isArray(rule.when?.noneOf) ? rule.when.noneOf.join(', ') : '';

  const notRaw = rule.__notRaw !== undefined ? rule.__notRaw : (rule.when && rule.when.not ? JSON.stringify(rule.when.not, null, 2) : '');

  const translateX = (rule.translate && typeof rule.translate.x === 'number' && Number.isFinite(rule.translate.x)) ? String(rule.translate.x) : '';

  const translateY = (rule.translate && typeof rule.translate.y === 'number' && Number.isFinite(rule.translate.y)) ? String(rule.translate.y) : '';

  const translateMode = rule.translate && typeof rule.translate.mode === 'string' ? rule.translate.mode : 'add';

  const rotateDeg = (rule.rotate && typeof rule.rotate.degrees === 'number' && Number.isFinite(rule.rotate.degrees)) ? String(rule.rotate.degrees) : '';

  const rotateMode = rule.rotate && typeof rule.rotate.mode === 'string' ? rule.rotate.mode : 'add';

  const scaleFactor = (rule.scale && typeof rule.scale.factor === 'number' && Number.isFinite(rule.scale.factor)) ? String(rule.scale.factor) : '';

  const scaleMode = rule.scale && typeof rule.scale.mode === 'string' ? rule.scale.mode : 'multiply';

  const layers = new Set();

  if (rule.target?.layer) layers.add(rule.target.layer);

  if (Array.isArray(rule.target?.layers)) rule.target.layers.forEach((l) => layers.add(l));

  const selectedLayers = Array.from(layers).map((l) => String(l || '').trim()).filter((l) => l.length > 0);

  const checkboxHtml = layerNames.length

    ? layerNames.map((name) => {

        const checked = selectedLayers.includes(name) ? 'checked' : '';

        return `<label class="tf-layer-chip"><input type="checkbox" class="tf-layer-toggle" data-layer="${escapeHtml(name)}" ${checked}>${escapeHtml(name)}</label>`;

      }).join('')

    : '<div class="muted">No named layers yet.</div>';

  const extraLayers = selectedLayers.filter((name) => !layerNames.includes(name));

  const extraHtml = extraLayers.length

    ? `<div class="tf-extra-layers">${extraLayers.map((name) => `<span class="tf-extra-chip" data-layer="${escapeHtml(name)}">${escapeHtml(name)} <button type="button" class="tf-extra-remove" title="Remove">×</button></span>`).join('')}</div>`

    : '';

  return `

    <div class="tf-card${highlight}" data-index="${idx}">

      <div class="tf-card-head">

        <div class="tf-summary">${escapeHtml(summarizeTransformRule(rule))}</div>

        <div class="tf-card-actions">

          <button type="button" class="tf-duplicate" title="Duplicate rule">Duplicate</button>

          <button type="button" class="tf-delete" title="Remove rule">Delete</button>
      ...
      <div class="tf-field">
        ...
        <input type="text" class="tf-description" placeholder="Optional label" value="${description}">
      ...
      <div class="tf-meta">
        ...
        <input type="number" class="tf-priority" placeholder="0" value="${priorityStr}">

        <span class="tf-id">ID: ${idStr || '—'}</span>
      ...
      <div class="tf-field">
        ...
        <div class="tf-layer-grid">${checkboxHtml}</div>

        ${extraHtml}

        <div class="tf-layer-add">

          <input type="text" class="tf-layer-manual" placeholder="Add another layer name">

          <button type="button" class="tf-layer-add-btn">Add</button>

        </div>

      </div>

      <div class="tf-field tf-targets">

        <label>Limit to trait values (comma separated)</label>

        <input type="text" class="tf-target-values" placeholder="Layer:Value" value="${escapeHtml(valuesStr)}">

        <label>Limit to filenames (comma separated)</label>

        <input type="text" class="tf-target-filenames" placeholder="asset.png" value="${escapeHtml(filenamesStr)}">

      </div>

      <div class="tf-field tf-conditions">

        <label>Conditions (trait strings)</label>

        <div class="tf-condition-grid">

          <div>

            <span>anyOf</span>

            <input type="text" class="tf-cond-any" placeholder="Layer:Value" value="${escapeHtml(anyOfStr)}">
          ...
          <div>
            ...
            <input type="text" class="tf-cond-all" placeholder="Layer:Value" value="${escapeHtml(allOfStr)}">
          ...
          <div>
            ...
            <input type="text" class="tf-cond-none" placeholder="Layer:Value" value="${escapeHtml(noneOfStr)}">

          </div>

        </div>

        <label>not (advanced JSON)</label>

        <textarea class="tf-cond-not" placeholder="{ "anyOf": ["Layer:Value"] }">${escapeHtml(notRaw || '')}</textarea>

      </div>

      <div class="tf-field tf-transform">

        <label>Translate (pixels)</label>

        <div class="tf-transform-grid">

          <label>X<input type="number" class="tf-translate-x" placeholder="0" value="${translateX}"></label>

          <label>Y<input type="number" class="tf-translate-y" placeholder="0" value="${translateY}"></label>

          <label>Mode

            <select class="tf-translate-mode">

              <option value="add" ${translateMode === 'add' ? 'selected' : ''}>add</option>

              <option value="set" ${translateMode === 'set' ? 'selected' : ''}>set</option>
      ...
      <div class="tf-field tf-transform">
        ...
        <div class="tf-transform-grid">
          ...
          <label>θ<input type="number" class="tf-rotate-deg" placeholder="0" value="${rotateDeg}"></label>

          <label>Mode

            <select class="tf-rotate-mode">

              <option value="add" ${rotateMode === 'add' ? 'selected' : ''}>add</option>

              <option value="set" ${rotateMode === 'set' ? 'selected' : ''}>set</option>
      ...
      <div class="tf-field tf-transform">
        ...
        <div class="tf-transform-grid">
          ...
          <label>×<input type="number" class="tf-scale-factor" placeholder="1" step="0.01" value="${scaleFactor}"></label>

          <label>Mode

            <select class="tf-scale-mode">

              <option value="multiply" ${scaleMode === 'multiply' ? 'selected' : ''}>multiply</option>

              <option value="set" ${scaleMode === 'set' ? 'selected' : ''}>set</option>
      ...
      <div class="tf-errors" role="alert"></div>
...
function handleTransformInput(event) {
  ...
  if (!target || !transformModal) return;

  const card = target.closest('.tf-card');

  if (!card) return;

  const idx = Number(card.getAttribute('data-index'));

  const rule = transformDraft[idx];

  if (!rule) return;

  if (target.classList.contains('tf-description')) {

    rule.description = target.value.trim() || undefined;

  } else if (target.classList.contains('tf-priority')) {

    const num = Number(target.value);

    rule.priority = Number.isFinite(num) ? num : undefined;

  } else if (target.classList.contains('tf-target-values')) {

    ensureTransformTarget(rule);

    const vals = parseListInput(target.value);

    if (vals.length) rule.target.values = vals; else delete rule.target.values;

  } else if (target.classList.contains('tf-target-filenames')) {

    ensureTransformTarget(rule);

    const vals = parseListInput(target.value);

    if (vals.length) rule.target.filenames = vals; else delete rule.target.filenames;

  } else if (target.classList.contains('tf-cond-any')) {

    if (!rule.when) rule.when = {};

    const vals = parseListInput(target.value);

    if (vals.length) rule.when.anyOf = vals; else delete rule.when.anyOf;

    if (rule.when && Object.keys(rule.when).length === 0) rule.when = undefined;

  } else if (target.classList.contains('tf-cond-all')) {

    if (!rule.when) rule.when = {};

    const vals = parseListInput(target.value);

    if (vals.length) rule.when.allOf = vals; else delete rule.when.allOf;

    if (rule.when && Object.keys(rule.when).length === 0) rule.when = undefined;

  } else if (target.classList.contains('tf-cond-none')) {

    if (!rule.when) rule.when = {};

    const vals = parseListInput(target.value);

    if (vals.length) rule.when.noneOf = vals; else delete rule.when.noneOf;

    if (rule.when && Object.keys(rule.when).length === 0) rule.when = undefined;

  } else if (target.classList.contains('tf-cond-not')) {

    rule.__notRaw = target.value;

  } else if (target.classList.contains('tf-translate-x')) {

    if (!rule.translate) rule.translate = {};

    const val = target.value;

    if (val === '') delete rule.translate.x; else {

      const num = Number(val);

      if (Number.isFinite(num)) rule.translate.x = num; else delete rule.translate.x;

    }

    if (rule.translate && rule.translate.x === undefined && rule.translate.y === undefined) delete rule.translate;

  } else if (target.classList.contains('tf-translate-y')) {

    if (!rule.translate) rule.translate = {};

    const val = target.value;

    if (val === '') delete rule.translate.y; else {

      const num = Number(val);

      if (Number.isFinite(num)) rule.translate.y = num; else delete rule.translate.y;

    }

    if (rule.translate && rule.translate.x === undefined && rule.translate.y === undefined) delete rule.translate;

  } else if (target.classList.contains('tf-rotate-deg')) {
    ...
    if (val === '') {
      ...
      if (Number.isFinite(num)) {
        ...
        if (!rule.rotate) rule.rotate = { degrees: num };

        else rule.rotate.degrees = num;
  ...
  } else if (target.classList.contains('tf-scale-factor')) {
    ...
    if (val === '') {
      ...
      if (Number.isFinite(num)) {
        ...
        if (!rule.scale) rule.scale = { factor: num };

        else rule.scale.factor = num;
  ...
  updateTransformSummaryForCard(card, rule);
...
function handleTransformChange(event) {
  ...
  if (!card) {
    ...
    if (target.classList.contains('tf-modal-backdrop')) closeTransformModal();
  ...
  if (target.classList.contains('tf-layer-toggle')) {

    const name = String(target.getAttribute('data-layer') || '').trim();
    ...
    if (target.checked) layers.add(name); else layers.delete(name);

    setTransformLayers(rule, Array.from(layers));

  } else if (target.classList.contains('tf-translate-mode')) {

    if (!rule.translate) rule.translate = {};

    rule.translate.mode = target.value === 'set' ? 'set' : 'add';

  } else if (target.classList.contains('tf-rotate-mode')) {

    if (!rule.rotate) rule.rotate = { degrees: 0 };

    rule.rotate.mode = target.value === 'set' ? 'set' : 'add';

  } else if (target.classList.contains('tf-scale-mode')) {

    if (!rule.scale) rule.scale = { factor: 1 };

    rule.scale.mode = target.value === 'set' ? 'set' : 'multiply';
...
function handleTransformClick(event) {
  ...
  if (target.classList.contains('tf-close') || target.classList.contains('tf-cancel')) {
  ...
  if (target.classList.contains('tf-modal-backdrop')) {
  ...
  if (target.classList.contains('tf-add-rule')) {

    event.preventDefault();

    addTransformRule(transformFilterLayer || getTransformLayerNames()[0] || undefined);

    return;

  }

  if (target.classList.contains('tf-save')) {
  ...
  if (target.classList.contains('tf-delete')) {

    event.preventDefault();

    deleteTransformRule(idx);

    return;

  }

  if (target.classList.contains('tf-duplicate')) {

    event.preventDefault();

    duplicateTransformRule(idx);

    return;

  }

  if (target.classList.contains('tf-layer-add-btn')) {

    event.preventDefault();

    const input = card.querySelector('.tf-layer-manual');
  ...
  if (target.classList.contains('tf-extra-remove')) {

    event.preventDefault();

    const name = String(target.parentElement?.getAttribute('data-layer') || '').trim();
...
function addTransformRule(initialLayer) {

  const draft = prepareTransformRuleForEditing({ target: initialLayer ? { layer: initialLayer } : {} });

  transformDraft.push(draft);
...
function duplicateTransformRule(index) {

  const rule = transformDraft[index];

  if (!rule) return;

  const copy = prepareTransformRuleForEditing(rule);

  copy.id = createTransformRuleId();

  transformDraft.splice(index + 1, 0, copy);
...
function deleteTransformRule(index) {
  ...
  if (index < 0 || index >= transformDraft.length) return;

  transformDraft.splice(index, 1);
...
function ensureTransformTarget(rule) {
...
function setTransformLayers(rule, layers) {

  ensureTransformTarget(rule);

  const uniq = Array.from(new Set((layers || []).map((name) => String(name || '').trim()).filter((name) => name.length > 0)));

  if (uniq.length === 0) {

    delete rule.target.layer;

    delete rule.target.layers;

  } else if (uniq.length === 1) {

    rule.target.layer = uniq[0];

    rule.target.layers = [uniq[0]];

  } else {

    delete rule.target.layer;

    rule.target.layers = uniq;
...
function updateTransformSummaryForCard(card, rule) {

  const summary = card.querySelector('.tf-summary');

  if (summary) summary.textContent = summarizeTransformRule(rule);

}



function finalizeTransformDraft() {
  ...
  transformDraft.forEach((rule, idx) => {

    const clone = deepClone(rule);

    const rawNot = typeof clone.__notRaw === 'string' ? clone.__notRaw.trim() : '';

    if (!clone.when) clone.when = {};

    if (rawNot) {

      try {

        clone.when.not = JSON.parse(rawNot);

      } catch (err) {

        errors.push(`Rule ${idx + 1}: NOT condition must be valid JSON.`);

        perCardErrors.push({ index: idx, message: 'NOT condition must be valid JSON.' });

      }

    } else if (clone.when) {

      delete clone.when.not;

    }

    delete clone.__notRaw;

    finalRules.push(normalizeTransformRule(clone));

  });

  return { finalRules, errors, perCardErrors };

}



function normalizeTransformRule(rule) {

  const cleaned = deepClone(rule || {});

  if (!cleaned.target) cleaned.target = {};

  const layers = [];

  if (typeof cleaned.target.layer === 'string') layers.push(cleaned.target.layer);

  if (Array.isArray(cleaned.target.layers)) layers.push(...cleaned.target.layers);

  const uniqLayers = Array.from(new Set(layers.map((name) => String(name || '').trim()).filter((name) => name.length > 0)));

  if (uniqLayers.length === 0) {

    delete cleaned.target.layer;

    delete cleaned.target.layers;

  } else if (uniqLayers.length === 1) {

    cleaned.target.layer = uniqLayers[0];

    cleaned.target.layers = [uniqLayers[0]];

  } else {

    delete cleaned.target.layer;

    cleaned.target.layers = uniqLayers;

  }

  if (Array.isArray(cleaned.target.values)) {

    cleaned.target.values = cleaned.target.values.map((v) => String(v || '').trim()).filter((v) => v.length > 0);

    if (!cleaned.target.values.length) delete cleaned.target.values;

  }

  if (Array.isArray(cleaned.target.filenames)) {

    cleaned.target.filenames = cleaned.target.filenames.map((v) => String(v || '').trim()).filter((v) => v.length > 0);

    if (!cleaned.target.filenames.length) delete cleaned.target.filenames;

  }

  if (cleaned.when) {

    if (Array.isArray(cleaned.when.anyOf)) {

      cleaned.when.anyOf = cleaned.when.anyOf.map((v) => String(v || '').trim()).filter((v) => v.length > 0);

      if (!cleaned.when.anyOf.length) delete cleaned.when.anyOf;

    }

    if (Array.isArray(cleaned.when.allOf)) {

      cleaned.when.allOf = cleaned.when.allOf.map((v) => String(v || '').trim()).filter((v) => v.length > 0);

      if (!cleaned.when.allOf.length) delete cleaned.when.allOf;

    }

    if (Array.isArray(cleaned.when.noneOf)) {

      cleaned.when.noneOf = cleaned.when.noneOf.map((v) => String(v || '').trim()).filter((v) => v.length > 0);

      if (!cleaned.when.noneOf.length) delete cleaned.when.noneOf;

    }

    if (!cleaned.when.not || typeof cleaned.when.not !== 'object' || Object.keys(cleaned.when.not).length === 0) delete cleaned.when.not;

    if (Object.keys(cleaned.when).length === 0) delete cleaned.when;

  }

  if (cleaned.translate) {

    const hasX = typeof cleaned.translate.x === 'number' && Number.isFinite(cleaned.translate.x);

    const hasY = typeof cleaned.translate.y === 'number' && Number.isFinite(cleaned.translate.y);

    if (!hasX) delete cleaned.translate.x;

    if (!hasY) delete cleaned.translate.y;
    ...
    if (!hasX && !hasY) {
      ...
      cleaned.translate.mode = cleaned.translate.mode === 'set' ? 'set' : 'add';
  ...
  if (cleaned.rotate) {
    ...
    if (typeof cleaned.rotate.degrees !== 'number' || !Number.isFinite(cleaned.rotate.degrees)) {

      delete cleaned.rotate;

    } else {

      cleaned.rotate.mode = cleaned.rotate.mode === 'set' ? 'set' : 'add';
  ...
  if (cleaned.scale) {
    ...
    if (typeof cleaned.scale.factor !== 'number' || !Number.isFinite(cleaned.scale.factor) || cleaned.scale.factor <= 0) {

      delete cleaned.scale;

    } else {

      cleaned.scale.mode = cleaned.scale.mode === 'set' ? 'set' : 'multiply';

      cleaned.scale.factor = Math.max(0.001, cleaned.scale.factor);

    }

  }

  if (typeof cleaned.priority !== 'number' || !Number.isFinite(cleaned.priority)) delete cleaned.priority;
  ...
  if (typeof cleaned.id !== 'string' || !cleaned.id) cleaned.id = createTransformRuleId();
...
function validateTransformDraft(rules) {
  ...
  rules.forEach((rule, idx) => {
    ...
    if (!layers.length) {
      ...
      issues.push(`Rule ${idx + 1} must target at least one layer.`);

    }

    const hasTranslate = !!rule.translate;

    const hasRotate = !!rule.rotate;

    const hasScale = !!rule.scale;

    if (!hasTranslate && !hasRotate && !hasScale) {

      issues.push(`Rule ${idx + 1} must configure translate, rotate, or scale.`);

    }

    if (rule.scale && (typeof rule.scale.factor !== 'number' || !Number.isFinite(rule.scale.factor) || rule.scale.factor <= 0)) {

      issues.push(`Rule ${idx + 1} scale factor must be greater than 0.`);
...
function showTransformBanner(message, tone) {

  if (!transformBannerEl) return;

  transformBannerEl.textContent = message;

  transformBannerEl.classList.remove('tf-banner-error', 'tf-banner-success');

  transformBannerEl.classList.add(tone === 'success' ? 'tf-banner-success' : 'tf-banner-error');

  transformBannerEl.hidden = false;

}



function clearTransformBanner() {

  if (!transformBannerEl) return;

  transformBannerEl.textContent = '';

  transformBannerEl.hidden = true;
...
function clearTransformCardErrors() {

  if (!transformModal) return;

  transformModal.querySelectorAll('.tf-errors').forEach((el) => {
...
function applyTransformCardErrors(list) {

  if (!transformModal) return;

  list.forEach(({ index, message }) => {

    const card = transformModal.querySelector(`.tf-card[data-index="${index}"]`);

    if (!card) return;

    const el = card.querySelector('.tf-errors');
...
function applyTransformDraft() {
  ...
  const { finalRules, errors, perCardErrors } = finalizeTransformDraft();

  const validation = validateTransformDraft(finalRules);

  const allErrors = [...errors, ...validation];

  clearTransformCardErrors();

  if (perCardErrors.length) applyTransformCardErrors(perCardErrors);

  if (allErrors.length) {

    showTransformBanner(allErrors.join(' '), 'error');

    return;

  }

  const base = currentConfig && currentConfig.rules ? deepClone(currentConfig.rules) : {};

  base.transforms = deepClone(finalRules);
  ...
  if (textArea) {
    ...
    const merged = { ...parsed, ...base, transforms: deepClone(finalRules) };

    currentConfig.rules = merged;

    textArea.value = JSON.stringify(merged, null, 2);

  } else {

    currentConfig.rules = base;

  }

  closeTransformModal();

  updateTransformButtonBadges();

  log('Transforms updated.');

}



function updateTransformButtonBadges() {

  if (!tbody || !tbody.querySelectorAll) return;

  const buttons = tbody.querySelectorAll('.btn-transforms');

  buttons.forEach((btn) => {

    const row = btn.closest('tr[data-type="layer"]');
    ...
    if (row) {
      ...
      name = input && typeof input.value === 'string' ? input.value.trim() : '';

    }

    const count = transformCountForLayer(name);

    const label = count > 0 ? `Transforms (${count})` : 'Transforms';

    const title = count > 0 ? `${count} rule${count === 1 ? '' : 's'} targeting this layer` : 'No transform rules targeting this layer yet';
...
function layerRowTemplate(idx, layer) {
  ...
  const transformCount = transformCountForLayer(layerName);

  const transformLabel = transformCount > 0 ? `Transforms (${transformCount})` : 'Transforms';

  const transformTitle = transformCount > 0 ? `${transformCount} rule${transformCount === 1 ? '' : 's'} targeting this layer` : 'No transform rules targeting this layer yet';
  ...
  return `
    ...
    <tr data-type="layer" data-idx="${idx}" style="border-bottom:1px solid #21262d;">
      ...
      <td style="padding:4px 6px;">
        ...
        <button class="btn-transforms" data-layer-name="${escapeHtml(layerName)}" title="${escapeHtml(transformTitle)}" style="padding:4px 6px; background:#30363d; margin-left:6px;">${escapeHtml(transformLabel)}</button>
...
function bindLayerTableEvents() {
  ...
  tbody.addEventListener('click', (e) => {
    ...
    } else if (target.classList.contains('btn-transforms')) {

      const inputName = tr.querySelector('.layer-name') as HTMLInputElement | null;

      const layerName = inputName && typeof inputName.value === 'string' ? inputName.value.trim() : '';

      openTransformModal(layerName);
  ...
  tbody.addEventListener('input', (e) => {
    ...
    if (target && target.classList && target.classList.contains('layer-name')) {

      const row = target.closest('tr[data-type="layer"]') as HTMLElement | null;

      if (row) {

        const btn = row.querySelector('.btn-transforms') as HTMLButtonElement | null;

        if (btn && target instanceof HTMLInputElement) {
...
function populateFormFromConfig(cfg) {
  ...
  document.getElementById('rules-json').value = JSON.stringify(cfg.rules || { mutuallyExclusive: [], requires: [], maxOccurrences: [], transforms: [] }, null, 2);
...
const HELP_TOPICS = {
  ...
  'rules-advanced': {
    ...
    body: `
      ...
      <p>Rules operate on trait strings in the form <code>Layer:Value</code> (e.g., <code>Eyes:Laser</code>). Four core sections:</p>
      ...
      <ol>
        ...
        <li><b>transforms</b>: instruct the compositor to translate, rotate, or scale specific layers when conditions match.<br/>

        <pre><code>"transforms": [

  {

    "when": { "anyOf": ["Layout:Vertical"] },

    "target": { "layers": ["Name","ID Photo","Date"] },

    "translate": { "x": 40, "y": -120, "mode": "set" }
...

  {
    ...
    "target": { "layers": ["Accent"] },

    "rotate": { "degrees": 90, "mode": "set" }
      ...
      <ol>
        ...
        <li>Use <b>transforms</b> to reposition assets or apply alternate orientations.</li>
