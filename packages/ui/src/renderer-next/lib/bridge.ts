...
export interface FoundryBridge {
  ...
  readFileBase64(relativePath: string, maxBytes?: number): Promise<{ ok: boolean; base64?: string; mime?: string; error?: string }>;
  ...
  renameFileExact(from: string, to: string): Promise<{ ok: boolean; renamed?: number; error?: string }>;
