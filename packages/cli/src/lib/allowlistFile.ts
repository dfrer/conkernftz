import type { AllowlistEntry } from '@conkernftz/chain-evm';

/**
 * Parse an allowlist from a CSV or JSON file into `AllowlistEntry[]` for the Merkle builder.
 *
 * - **CSV**: one `address,maxQty` per line. A header row (`address,maxQty` / `address,qty`) is
 *   skipped if present; blank lines and `#` comments are ignored.
 * - **JSON**: an array of `{ address, maxQty }` (also accepts `qty` / `quantity` aliases), or an
 *   object map `{ "0xabc...": 3, ... }`.
 *
 * Validation of address/quantity is left to the Merkle helper (which throws on bad/duplicate
 * entries) so there is one source of truth for the rules.
 */
export function parseAllowlist(text: string, format: 'csv' | 'json'): AllowlistEntry[] {
  return format === 'json' ? parseJson(text) : parseCsv(text);
}

/** Pick the format from a file extension (`.json` → json, else csv). */
