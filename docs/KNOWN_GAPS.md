...
| Mint experience | Rip animation | Done (U3.5) | **Grab-and-pull** the sealed pack to rip it (click/Enter also work); the card backs then **rise out of a torn-open pack** and fan, via a deterministic CSS sequence. Uses an optional `<packId>-open` library variant (bundled for CONKERCO); packs without one fall back to a simple open. Animation feel/timing is owner-tunable. |
