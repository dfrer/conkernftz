...
| Mint site (generated) | Host / deploy | Partial | P4 — **Vercel + Netlify** (CLI + the user's token; Netlify needs a pre-created site ID since deploy is non-interactive) and **IPFS (Pinata) + Arweave (Irys)** (in-process, reusing the project's storage provider → gateway URL). Host picker in the Site ▸ Host panel. Still pending: **GitHub Pages** + **custom-domain wiring**. First `npx vercel`/`netlify` run is slow (fetches the CLI). |
