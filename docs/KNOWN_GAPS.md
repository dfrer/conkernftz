...
| Mint experience | Pack art | Partial | Owner-supplied finished pack art is now used as the sealed pack — **Mint FX ▸ Pack & card art** loads a project image into the config (as a self-contained data URL → renders in preview + ships in the exported site). Remaining: a large pack image bloats `foundry.config.json` (~+2 MB base64); future = store a project-relative path + resolve to a data URL at render/export instead of embedding. (Owner makes the art — see [[owner-makes-the-art]].) |
| Mint experience | Card backs | Partial | A single custom card-back image is now supported (**Mint FX ▸ Pack & card art**, `backArt`). Per-rarity / rules-driven flip sides still pending. |
