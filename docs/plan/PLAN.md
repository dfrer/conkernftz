...
| OC-3b | **Live token-tier in the mint widget** — parse the mint receipt → token id → tier and drive the reveal. Devnet-gated (needs a real mint receipt; `MintLive` has no token id without one). | ☐ |
