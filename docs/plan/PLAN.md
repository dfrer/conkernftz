...
| OC-2 | Reveal flow UX — full upload-metadata → reveal → freeze path, in-app. EVM reveal now **auto-fills the baseURI from the upload manifest** + a guided **upload → reveal → freeze stepper** showing live progress (no hand-copied CID). | ◐ |
